...
if __name__ == '__main__':
    SEED, n = 1, 2
    toks, tags, groups = [l[:3000] for l in load(
        tag_prefix_masks=[])]  # load()  # load(tag_prefix_masks=[])  #[l[:3000] for l in load(tag_prefix_masks=[])]    #
    paramspace = [  # dict(clf=IntergrammTagger),
        {},
        dict(augment_setvalued_targets='True',
             corenlp_train_params=['--arch', 'left5words,suffix(1),prefix(1),suffix(2),prefix(2),suffix(3),prefix(3)'],
             filter_tags=()),
    ...
    with shelve.open(f'result_seed={SEED}_n={len(toks)}_smac-validation.pkl') as db:
                ...
                if n > len(scores):
                    try:
                        new_scores = cross_val_score(clf, toks, tags, groups,
                                                     cv=KFoldInDocSplitter(k=2, seed=0), n_jobs=1)
